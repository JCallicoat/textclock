...
struct xcbft_face_holder xcbft_query_by_char_support(FcChar32,
                                                     const FcPattern *, long);
...
struct xcbft_glyphset_and_advance xcbft_load_glyphset(xcb_connection_t *,
                                                      struct xcbft_face_holder,
                                                      struct utf_holder, long);
FT_Vector xcbft_load_glyph(xcb_connection_t *, xcb_render_glyphset_t, FT_Face,
                           int);
...
xcb_pixmap_t
xcbft_create_text_pixmap(xcb_connection_t *c, struct utf_holder text,
                         xcb_render_color_t text_color,
                         xcb_render_color_t background_color,
                         struct xcbft_patterns_holder font_patterns, long dpi) {
  ...
  FT_Vector advance = xcbft_draw_text(c, pmap, 0.2 * pix_size,
                                      0.2 * pix_size + pix_size, // x, y
                                      text, text_color, faces, dpi);

  resize_pmap = xcb_generate_id(c);
  width = advance.x + pix_size * 0.4; // 0.2 on both sides
  xcb_create_pixmap(c, screen->root_depth, resize_pmap, screen->root, width,
                    height);
 ...
 */
struct xcbft_face_holder
xcbft_query_by_char_support(FcChar32 character, const FcPattern *copy_pattern,
                            long dpi) {
  FcBool status;
  FcResult result;
  FcCharSet *charset;
...
struct xcbft_patterns_holder xcbft_query_fontsearch_all(FcStrSet *queries) {
    ...
    if (font_pattern != NULL) {
      if (font_patterns.length + 1 > current_allocated) {
        ...
        font_patterns.patterns = realloc(
            font_patterns.patterns, sizeof(FcPattern *) * current_allocated);
      }
...
double xcbft_get_pixel_size(struct xcbft_patterns_holder patterns) {
  ...
  for (i = 0; i < patterns.length; i++) {
    result =
        FcPatternGet(patterns.patterns[i], FC_PIXEL_SIZE, 0, &fc_pixel_size);
...
struct xcbft_face_holder xcbft_load_faces(struct xcbft_patterns_holder patterns,
                                          long dpi) {
  int i;
  struct xcbft_face_holder faces;
  FcResult result;
  ...
  for (i = 0; i < patterns.length; i++) {
    ...
    // load the face
    error = FT_New_Face(library, (const char *)fc_file.u.s, fc_index.u.i,
    ...
    // pixel_size/ (dpi/72.0)
    FT_Set_Char_Size(faces.faces[faces.length], 0,
                     (fc_pixel_size.u.d / ((double)dpi / 72.0)) * 64, dpi, dpi);
    if (error != FT_Err_Ok) {
      perror(NULL);
...
FcStrSet *xcbft_extract_fontsearch_list(char *string) {
...
void xcbft_patterns_holder_destroy(struct xcbft_patterns_holder patterns) {
  int i = 0;
...
void xcbft_face_holder_destroy(struct xcbft_face_holder faces) {
  int i = 0;
...
FT_Vector xcbft_draw_text(xcb_connection_t *c,    // conn
                          ...
                          struct xcbft_face_holder faces, long dpi) {
  ...
  xcb_render_util_composite_text_stream_t *ts =
      xcb_render_util_composite_text_stream(glyphset_advance.glyphset,
                                            text.length, 0);
...
xcb_render_picture_t xcbft_create_pen(xcb_connection_t *c,
                                      xcb_render_color_t color) {
  xcb_render_pictforminfo_t *fmt;
  const xcb_render_query_pict_formats_reply_t *fmt_rep =
      xcb_render_util_query_formats(c);
  ...
  xcb_render_create_picture(c, picture, pm, fmt->id, XCB_RENDER_CP_REPEAT,
                            values);

  xcb_rectangle_t rect = {.x = 0, .y = 0, .width = 1, .height = 1};

  xcb_render_fill_rectangles(c, XCB_RENDER_PICT_OP_OVER, picture, color, 1,
                             &rect);
...
struct xcbft_glyphset_and_advance
xcbft_load_glyphset(xcb_connection_t *c, struct xcbft_face_holder faces,
                    struct utf_holder text, long dpi) {
  unsigned int i, j;
  int glyph_index;
  ...
  for (i = 0; i < text.length; i++) {
    ...
    if (glyph_index != 0) {
      ...
      if (faces_for_unsupported.length > 0) {
        glyph_index =
            FT_Get_Char_Index(faces_for_unsupported.faces[0], text.str[i]);
      }
      if (glyph_index == 0) {
        if (faces_for_unsupported.length > 0) {
          xcbft_face_holder_destroy(faces_for_unsupported);
        }
        faces_for_unsupported =
            xcbft_query_by_char_support(text.str[i], NULL, dpi);
      }
      if (faces_for_unsupported.length == 0) {
        ...
        FT_Set_Char_Size(
            faces_for_unsupported.faces[0], 0,
            (faces.faces[0]->size->metrics.x_ppem / ((double)dpi / 72.0)) * 64,
            dpi, dpi);

        glyph_advance = xcbft_load_glyph(c, gs, faces_for_unsupported.faces[0],
                                         text.str[i]);
        total_advance.x += glyph_advance.x;
        total_advance.y += glyph_advance.y;
      }
...
FT_Vector xcbft_load_glyph(xcb_connection_t *c, xcb_render_glyphset_t gs,
                           FT_Face face, int charcode) {
  uint32_t gid;
  int glyph_index;
  ...
  for (y = 0; y < ginfo.height; y++)
    memcpy(tmpbitmap + y * stride, bitmap->buffer + y * ginfo.width,
           ginfo.width);

  xcb_render_add_glyphs_checked(c, gs, 1, &gid, &ginfo, stride * ginfo.height,
                                tmpbitmap);
...
long xcbft_get_dpi(xcb_connection_t *c) {
  ...
  if (xrm_db != NULL) {
    ...
    if (i < 0) {
      fprintf(stderr, "Could not fetch value of Xft.dpi from Xresources "
                      "falling back to highest dpi found\n");
    } else {
      return dpi;
    }
  } else {
    fprintf(stderr, "Could not open Xresources database falling back to "
                    "highest dpi found\n");
  }
